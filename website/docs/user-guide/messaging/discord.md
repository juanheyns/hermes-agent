...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Hermes. `"all"` — accept all bot messages. Note: when bot messages are accepted, emote-only messages from bots (e.g. just `🫡` or `:saluting_face:`, with or without an `@mention` prefix) are always dropped to prevent response loops between agents using emoji acks to signal end-of-conversation. |
